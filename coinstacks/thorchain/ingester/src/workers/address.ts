...
const onMessage = (worker: Worker) => async (message: Message) => {
  const { address, txid, internalTxs, client_id }: SyncTx = message.getContent()
...
const main = async () => {
  const worker = await Worker.init({
    queueName: 'queue.thorchain.txid.address',
    exchangeName: 'exchange.thorchain.tx.client',
  })
