...
import { OsmosisAPI, OsmosisAccount, Token } from './models'
...
@Route('api/v1')
@Tags('v1')
export class Osmosis extends Controller implements BaseAPI, OsmosisAPI {
  /**
  ...
  @Get('account/{pubkey}')
  async getAccount(@Path() pubkey: string): Promise<OsmosisAccount> {
    try {
