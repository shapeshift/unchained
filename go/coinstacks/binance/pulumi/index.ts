...
export = async (): Promise<Outputs> => {
  const { kubeconfig, config, namespace } = await getConfig('binance')

  const name = 'unchained'
  const asset = config.network !== 'mainnet' ? `binance-${config.network}` : 'binance'
  const outputs: Outputs = {}

  const provider = new k8s.Provider('kube-provider', { kubeconfig })

  const missingKeys: Array<string> = []
  const stringData = Object.keys(parse(readFileSync('../../../cmd/binance/sample.env'))).reduce((prev, key) => {
