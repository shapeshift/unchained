  ...
  async getGasFees(): Promise<GasFees> {
    try {
      ...
      const eip1559Fees = await (async () => {
        ...
        const feeHistory = (await this.provider.send('eth_feeHistory', [totalBlocks, 'latest', [1, 60, 90]])) as any
        ...
        for (let i = oldestBlock; i < latestBlock; i++) {
          ...
          blockHistory.push({
            ...
            maxPriorityFeePerGas: feeHistory.reward[index].map((r: any) => new BigNumber(r)),
          })
        }

        //console.log(blockHistory.map(block => ({ block: block.number, maxPriorityFees: block.maxPriorityFeePerGas.map((f: any) => f.toString())})))
