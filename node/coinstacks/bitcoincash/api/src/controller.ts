...
@Route('api/v1')
@Tags('v1')
export class BitcoinCash extends Controller implements BaseAPI, BitcoinCashAPI {
  /**
   ...
   *
   * Examples
   * 1. BitcoinCash (address)
   * 2. BitcoinCash (xpub)
   *
   * @param {string} pubkey account address or xpub
   *
   * @returns {Promise<BitcoinCashAccount>} account details
   *
  ...
  @Get('account/{pubkey}')
  async getAccount(@Path() pubkey: string): Promise<BitcoinCashAccount> {
    try {
      ...
      const addresses = data.tokens?.map<BitcoinCashAddress>((token) => ({
  ...
  @Response<InternalServerError>(500, 'Internal Server Error')
  @Get('tx/{txid}')
  async getTransaction(@Path() txid: string): Promise<BitcoinCashTx> {
    try {
  ...
  @Response<InternalServerError>(500, 'Internal Server Error')
  @Get('tx/{txid}/raw')
  async getRawTransaction(@Path() txid: string): Promise<BitcoinCashRawTx> {
    try {
      const data = await blockbook.getTransactionSpecific(txid)
      return data as BitcoinCashRawTx
    } catch (err) {
      throw handleError(err)
    }
  }
