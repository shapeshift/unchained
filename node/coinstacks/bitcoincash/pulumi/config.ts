...
export interface BitcoinCashConfig {
  kubeconfig: string
  config: Config
  namespace: string
}

export const getConfig = async (): Promise<BitcoinCashConfig> => {
  let config: Config
  try {
    config = new pulumi.Config('unchained').requireObject<Config>('bitcoincash')
  } catch (e) {
