...
export = async (): Promise<Outputs> => {
  ...
  const asset = config.network !== 'mainnet' ? `bitcoincash-${config.network}` : 'bitcoincash'
  ...
  if (existsSync('../.env')) {
    ...
    throw new Error(
      'you must run `cp sample.env .env` from the bitcoincash coinstack directory and fill out any empty values.'
    )
  }
