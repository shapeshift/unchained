...
export = async (): Promise<Outputs> => {
  ...
  const asset = config.network !== 'mainnet' ? `litecoin-${config.network}` : 'litecoin'
  ...
  if (existsSync('../.env')) {
    ...
    throw new Error(
      'you must run `cp sample.env .env` from the litecoin coinstack directory and fill out any empty values.'
    )
  }
