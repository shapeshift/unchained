...
@Route('api/v1')
@Tags('v1')
export class Litecoin extends Controller implements BaseAPI, LitecoinAPI {
  /**
   ...
   *
   * @returns {Promise<LitecoinAccount>} account details
   *
  ...
  @Get('account/{pubkey}')
  async getAccount(@Path() pubkey: string): Promise<LitecoinAccount> {
    try {
      ...
      const addresses = data.tokens?.map<LitecoinAddress>((token) => ({
  ...
  @Response<InternalServerError>(500, 'Internal Server Error')
  @Get('tx/{txid}')
  async getTransaction(@Path() txid: string): Promise<LitecoinTx> {
    try {
  ...
  @Response<InternalServerError>(500, 'Internal Server Error')
  @Get('tx/{txid}/raw')
  async getRawTransaction(@Path() txid: string): Promise<LitecoinRawTx> {
    try {
      const data = await blockbook.getTransactionSpecific(txid)
      return data as LitecoinRawTx
    } catch (err) {
      throw handleError(err)
    }
  }
