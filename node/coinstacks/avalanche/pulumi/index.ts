...
export = async (): Promise<Outputs> => {
  ...
  const coinServiceArgs =
    config.statefulService?.services?.map((service): CoinServiceArgs => {
      switch (service.name) {
        case 'daemon':
          return {
            ...service,
  ...
  return await deployCoinstack(kubeconfig, config, namespace, appName, coinstack, coinServiceArgs, sampleEnv, 'node')
}
