...
if (config.isLocal === false) {
  ...
  if (config.eks.autoscaling) {
    if (!config.eks.autoscaling.enabled) missingRequiredConfig.push('eks.autoscaling.enabled')
    if (!config.eks.autoscaling.minInstances) missingRequiredConfig.push('eks.autoscaling.minInstances')
    if (!config.eks.autoscaling.maxInstances) missingRequiredConfig.push('eks.autoscaling.maxInstances')
  }
}
